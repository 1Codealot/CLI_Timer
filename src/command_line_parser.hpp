...
static bool shouldPrompt(std::vector<std::string>& args)
{
	for (size_t i = 0; i < args.size(); ++i)
		if (args.at(i) == "--no_prompt") {
			if (shouldSave(args)) {
				std::cout
					<< "You are saving to a file; this means you will need prompting; therefore you will be prompted.\nHowever you can type 'save' if you do want to save to a file with `--no_prompt`"
					<< std::endl;
				return true;
			} else {
				return false;
	...
	for (size_t i = 0; i < args.size(); i++) {
		if (args.at(i) == "-b" || args.at(i) == "-B") {
			...
			if (getCubeType(args) == '3') {
				return { .on = true, .count = std::stoi(args.at(i).substr(5)) };
			} else {
				std::cout
					<< "Multiblind support is only for 3x3\nAlso you are insane for doing it on non 3x3s\n";
				return { .on = false, .count = 0 };
			}
		}
	}
		...
		if (args.at(i).substr(0, 2) == "-f" || args.at(i).substr(0, 2) == "-F") {
			if (getCubeType(args) == '3') {
				if (!blindfolded(args).on) {
					...
					std::cout
						<< "Incompatible arguments: -f cannot be used with non-blindfolded cubes.\n";
					return false;
				}
			} else {
...
static long getSeed(std::vector<std::string>& args)
{
	...
	if (hasNonNum) {
		std::clog
			<< "WARNING: Seed has non-numeric values (including \"-\").\nIt has been coverted to: "
			<< actual << ".\n";
	}
