...
enum class baseMoves { F, U, R, B, L, D };

enum class directions { CW, ACW, DOUBLE };

enum class wideSizes { NONE, WIDE, WIDE2 };
...
inline void createMove(puzzle_move& newMove, const char moveType)
{
	...
	case '4':
		if (newMove.base == baseMoves::U || newMove.base == baseMoves::F ||
			newMove.base == baseMoves::R) {
